from  data.Calendar.DateCalc import *
import re
import time
...
def borderDate(ui, month, day):  # 选中后加框
	...
	selected.setStyleSheet("QLabel{border:3px solid; border-radius: 5px; border-color:green}")
...
def displayDate(ui):
	...
	if ui.sender() in [None, ui.btnToday]:  # 设为今日
		...
		if ui.sender() in [ui.cblCentury, ui.cblYear, ui.cblMonth, ui.btnLastMonth, ui.btnNextMonth, ui.btnLastYear, ui.btnNextYear, ui.cblFindFestival]:  # 设为原公历日
			...
			if not re.search(r"<font style='font-size:20px; text-align:center; color:gray", selected.text()):  # 本月内
				ui.sender().setStyleSheet("QLabel{border:3px solid; border-radius: 5px; border-color:green}")  # 1E90FF 9400D3
			else:   # 跳转前后月
	...
	ui.labInfo.setText("<br/><br/><br/><br/>JDN {}<br/>{}<br/>{}<br/>{}月 星期{}<br/>{}{}<br/>{}年 〖{}〗<br/>{}月 {}日<br/><br/>".format(
		font(jdn, 12, "black"), font(difference, 36, "black"), nm, month+1, week, font(day, 50, "black"), font(ym+rq, 17, "black"), ngz, sxm, ygz, rgz))
